...
VisionInterpreter    — TFL JamCam images → Gemini Flash → anomalies
NumericInterpreter   — Air quality / weather / energy / env → z-score → anomalies
TextInterpreter      — GDELT news → Gemini Flash classification → anomalies
...
log = logging.getLogger("london.agents.interpreters")
...
class VisionInterpreter(BaseAgent):
    """Reads raw JamCam image messages and analyses them with Gemini Flash."""

    name = "vision_interpreter"
    _cycle_count = 0

    def __init__(self, board, graph, memory, coordinator=None):
        super().__init__(board, graph, memory, coordinator)
    ...
    async def run(self) -> None:
        ...
        # Process up to 5 cameras per cycle to conserve Gemini quota
        for msg in cam_messages[:5]:
            ...
            try:
                ...
                if response_text.startswith("ERROR:"):
                    log.warning("[VisionInterpreter] Gemini error for %s: %s", camera_id, response_text)
                    continue
...
class TextInterpreter(BaseAgent):
    """
    Reads #raw text observations (GDELT news) and classifies with Gemini Flash.
    ...
    async def _process_batch(
        ...
        if response_text.startswith("ERROR:"):
            log.warning("[TextInterpreter] Gemini error: %s", response_text)
            return
