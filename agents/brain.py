"""
Brain agent — the London Nervous System's synthesis engine.

Runs every 5 minutes. Reads all channels. Uses Gemini Pro to synthesize a
...
from ..core.config import GEMINI_PRO_MODEL, GEMINI_FLASH_MODEL
...
log = logging.getLogger("london.agents.brain")

_BRAIN_SYSTEM = """\
You are the analytical brain of London's Nervous System — a city-wide sensor network that
...
1. What is the SINGLE most interesting thing London's nervous system has detected right now?
...
You are reflecting on how London's Nervous System is performing over the past 6 hours.
...
class BrainAgent(BaseAgent):
    ...
    def __init__(self, board: MessageBoard, graph: LondonGraph, memory: MemoryManager) -> None:
    ...
    async def run(self) -> None:
        ...
        if response_text.startswith("ERROR:"):
            log.warning("[Brain] Gemini Pro error: %s", response_text)
            return
    ...
    async def _maybe_self_improve(self, now: datetime) -> None:
        ...
        if response_text.startswith("ERROR:"):
            log.warning("[Brain] Self-improvement Gemini error: %s", response_text)
            return
