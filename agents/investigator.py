...
log = logging.getLogger("london.agents.investigator")
...
PLANNING_SYSTEM = """You are the investigation planner for the London Nervous System — an AI monitoring London in real-time with 60+ data sources.
...
SYNTHESIS_SYSTEM = """You are synthesizing investigation findings for the London Nervous System — an AI that monitors all of London in real-time.
...
class InvestigatorAgent(BaseAgent):
    ...
    def __init__(self, board: MessageBoard, graph: LondonGraph, memory: MemoryManager | None = None):
    ...
    async def _plan(self, question: str, context: list[dict] | None = None, system_ctx: str = "") -> dict | None:
        ...
        raw = await self.call_gemini(
            prompt, model=GEMINI_FLASH_MODEL,
