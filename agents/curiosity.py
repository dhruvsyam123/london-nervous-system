...
Gemini to find patterns worth exploring.
...
class CuriosityAgent(BaseAgent):
    ...
    async def _juxtapose(self) -> None:
        """Pick 2-3 random data sources, fetch recent observations, ask Gemini what's interesting."""
...
{{"interesting": true/false, "observation": "...", "question": "...",
  "investigate_with": "agent name", "reasoning": "..."}}"""
        ...
        result = await self.call_gemini(prompt=prompt, model=GEMINI_FLASH_MODEL, trigger="curiosity:juxtapose")
    ...
    async def _temporal(self) -> None:
        ...
        prompt = f"""You are analyzing temporal patterns in London's {source} data.
...
{{"interesting": true/false, "observation": "...", "question": "...",
  "investigate_with": "agent name", "reasoning": "..."}}"""
        ...
        result = await self.call_gemini(prompt=prompt, model=GEMINI_FLASH_MODEL, trigger=f"curiosity:temporal:{source}")
        parsed = parse_json_response(result)
        if parsed and parsed.get("interesting"):
            target = self._resolve_agent(parsed.get("investigate_with", ""))
...
{{"interesting": true/false, "observation": "...", "question": "...",
  "investigate_with": "agent name", "reasoning": "..."}}"""
        ...
        result = await self.call_gemini(prompt=prompt, model=GEMINI_FLASH_MODEL, trigger=f"curiosity:spatial:{cell[:8]}")
        parsed = parse_json_response(result)
        if parsed and parsed.get("interesting"):
            target = self._resolve_agent(parsed.get("investigate_with", ""))
    ...
    async def _handle_investigation(self, inv_thread: list[dict]) -> None:
        ...
        # Use Gemini to reason about it and route to the best agent
...
{{"finding": "your analysis", "next_question": "refined question or null", "next_agent": "agent name or null"}}"""
        ...
        result = await self.call_gemini(prompt=prompt, model=GEMINI_FLASH_MODEL, trigger=f"curiosity:inv:{thread_id[:8]}")
        parsed = parse_json_response(result)
        if parsed:
...
async def run_curiosity_engine(
    board: MessageBoard, graph: LondonGraph, memory: MemoryManager,
) -> None:
