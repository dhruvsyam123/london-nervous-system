...
log = logging.getLogger("london.agents.chronicler")

_CHRONICLER_SYSTEM = """\
You are the long-term memory and narrative tracker for London's Nervous System.
...
Keep max 10 active narratives. Archive dead ones."""
        ...
        if response.startswith("ERROR:"):
            log.warning("[Chronicler] Gemini error: %s", response)
            return
    ...
    async def _weekly_reflection(self, now: datetime) -> None:
        ...
        prompt = f"""Produce a weekly reflection on what London's Nervous System has learned.
