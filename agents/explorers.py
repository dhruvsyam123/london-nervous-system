...
from ..core.config import GEMINI_FLASH_MODEL, GEMINI_PRO_MODEL, ENDPOINTS
...
log = logging.getLogger("london.agents.explorers")
...
You are an autonomous field investigator for London's sensor network. You receive a hypothesis
about what might be happening in London and plan a targeted investigation.
...
class Explorer(BaseAgent):
    ...
    1. Plans which APIs to query (Gemini Pro)
    2. Fetches data from those APIs
    3. Analyses results (Gemini Flash)
    ...
    async def investigate(self) -> None:
        ...
        # Step 2: Plan the investigation using Gemini Pro
        ...
        plan_response = await self.call_gemini(
            prompt=plan_prompt,
        ...
        async with aiohttp.ClientSession(
            ...
            headers={"User-Agent": "LondonNervousSystem/1.0"},
        ) as session:
            for q in queries:
                url = q.get("url", "")
        ...
        # Step 4: Analyse results with Gemini Flash
        ...
        analysis_response = await self.call_gemini(
