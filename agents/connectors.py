...
log = logging.getLogger("london.agents.connectors")
...
_NARRATIVE_SYSTEM = """\
You are a sophisticated urban intelligence analyst for London's real-time monitoring system.
...
street-level knowledge of London with analytical rigor.
"""

_NARRATIVE_PROMPT = """\
London's sensor network has detected the following co-located anomalies. Your task is to:
...
class NarrativeConnector(BaseAgent):
    ...
    Uses Gemini Pro to construct causal narratives and make predictions.
    ...
    async def run(self) -> None:
        ...
        for hypothesis_msg in spatial[:remaining]:
            ...
            if response_text.startswith("ERROR:"):
                log.warning("[NarrativeConnector] Pro call error: %s", response_text)
                continue
    ...
    async def _handle_investigation(self, inv_thread: list[dict]) -> None:
        ...
        response = await self.call_gemini(
            prompt=prompt, model=GEMINI_PRO_MODEL,
...
# Built-in causal templates for London
