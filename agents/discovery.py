...
log = logging.getLogger("london.agents.discovery")
...
London's nervous system has found these connections:
...
_IMPLICATION_PROMPT = """\
London's nervous system recently discovered this:
...
class DiscoveryEngine(BaseAgent):
    ...
    without requiring anomalies. Runs every 5 minutes using Gemini Flash.
    """
    ...
    async def _cross_domain_scan(self) -> None:
        ...
        if response.startswith("ERROR:"):
            log.warning("[DiscoveryEngine] Gemini error: %s", response)
            return
