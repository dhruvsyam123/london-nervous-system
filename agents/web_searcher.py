...
Uses Gemini's built-in Google Search grounding for reliable web results.
"""
...
log = logging.getLogger("london.agents.web_searcher")
...
class WebSearchAgent(BaseAgent):
    ...
    async def _investigate(self, inv_thread: list[dict]) -> None:
        ...
        # Generate search queries via Gemini Flash
...
["query 1", "query 2", "query 3"]"""
        ...
        result = await self.call_gemini(prompt=prompt, model=GEMINI_FLASH_MODEL, trigger="websearch:queries")
...
{{"finding": "concise synthesis of what you found (2-3 sentences)",
  ...
  "sources_useful": true/false}}"""
        ...
        result = await self.call_gemini(prompt=prompt, model=GEMINI_FLASH_MODEL, trigger="websearch:synthesize")
        parsed = parse_json_response(result)
