...
from ..core.config import GEMINI_FLASH_MODEL, ENDPOINTS
...
log = logging.getLogger("london.agents.validator")
...
class ValidatorAgent(BaseAgent):
    """
    Checks pending predictions. Uses sensor data + Gemini Flash to determine outcomes.
    ...
    async def _evaluate_prediction(
        ...
        """Gather evidence and use Gemini Flash to evaluate a prediction."""
        ...
        if response_text.startswith("ERROR:"):
            log.warning("[Validator] Gemini error for %s: %s", conn_id, response_text)
            return None
    ...
    async def _record_outcome(
        ...
        # We use board DB directly (would normally need an update method)
        if self.board._db:
            try:
