...
log = logging.getLogger("london.daemon")
...
_PROTECTED_FILES_NOTICE = (
    ...
    "  london/run.py, london/core/board.py, london/core/coordinator.py, "
    "london/core/scheduler.py, london/core/models.py\n"
    "PREFERRED: To register a new ingestor WITHOUT restarting, write a JSON entry to "
    "london/data/memory/pending_registrations.json:\n"
    '[{"task_name": "ingest_foo", "module": "london.ingestors.foo", "func": "ingest_foo", '
...
class Daemon:
    ...
    async def _spawn_for_task_errors(
        ...
        prompt = (
            f"The London Nervous System task '{task_name}' has failed {error_count} "
            f"consecutive times. Health info: {json.dumps(health_info)}\n\n"
            f"Please investigate the task code, check the error logs at "
            f"london/data/logs/london.log, and fix the issue.\n\n"
            f"Read london/ARCHITECTURE.md first for system overview."
        ...
        if entry and entry.module_name:
            ...
            context_files.append(f"london/ingestors/{source}.py")
    ...
    async def _spawn_for_accuracy_drop(
        ...
        prompt = (
            f"The London Nervous System's prediction accuracy has dropped to {accuracy:.0%}. "
            f"Metrics: {json.dumps(metrics)}\n\n"
            f"Please review the brain (london/agents/brain.py) and connectors "
            f"(london/agents/connectors.py) to identify why predictions are failing. "
            f"Check if the epistemic engine (london/core/epistemics.py) thresholds need adjustment.\n\n"
            f"Read london/ARCHITECTURE.md first for system overview."
            f"{_PROTECTED_FILES_NOTICE}"
        )
        ...
        context_files = [
            ...
            "london/agents/brain.py",
            "london/agents/connectors.py",
            "london/core/epistemics.py",
        ]
    ...
    async def _spawn_for_investigation(
        ...
        prompt = (
            f"An agent ({from_agent}) in the London Nervous System needs help investigating:\n\n"
            f"Question: {question}\n\n"
            f"Context: {context}\n\n"
            f"Read london/ARCHITECTURE.md for system overview. "
    ...
    async def _maybe_spawn_for_new_source(
        ...
        prompt = (
            ...
            f"following the BaseIngestor pattern. See london/ARCHITECTURE.md for how to "
            f"add an ingestor.\n\n"
            f"Steps:\n"
            f"1. Check if the API/data source exists and is accessible\n"
            f"2. Create a new ingestor file in london/ingestors/\n"
        ...
        context_files = [
            str(_ARCHITECTURE_PATH),
            "london/ingestors/air_quality.py",  # example ingestor
        ]
    ...
    async def _get_changed_py_files(self) -> list[str]:
        ...
        try:
            ...
            # Only .py files under london/
            return [f for f in all_changed if f.endswith(".py") and f.startswith("london/")]
    ...
    async def _git_auto_commit(self, changed_files: list[str], instance_id: str) -> None:
        ...
        try:
            ...
            # Stage the changed london/ .py files
            proc = await asyncio.create_subprocess_exec(
    ...
    async def _hot_reload(self, changed_files: list[str]) -> tuple[int, int]:
        ...
        for filepath in changed_files:
            ...
            if "core/" in filepath and filepath not in (
                "london/core/board.py", "london/core/coordinator.py",
                "london/core/scheduler.py", "london/core/models.py",
            ):
        ...
        all_agent_modules = {
            ...
            if m.startswith("london.agents.")
        }

        if base_changed:
            base_mod = "london.agents.base"
            ...
            for mod_name in agent_modules:
                if mod_name == "london.agents.base":
                    continue
                if mod_name in sys.modules:
                    try:
                        importlib.reload(sys.modules[mod_name])
