"""Live web dashboard — aiohttp server with WebSocket log streaming."""
...
log = logging.getLogger("london.dashboard")
...
class DashboardLogHandler(logging.Handler):
    ...
    def emit(self, record: logging.LogRecord) -> None:
        entry = {
            ...
            "name": record.name.replace("london.", ""),
...
class DashboardServer:
    """aiohttp web server serving the live dashboard."""
    ...
    async def start(self, board: MessageBoard, coordinator: Coordinator, memory: MemoryManager | None = None, graph: Any = None, retina: Any = None) -> None:
        ...
        app.router.add_get("/ask", self._serve_ask_page)
    ...
    async def _serve_ask_page(self, request: web.Request) -> web.Response:
        html_path = STATIC_DIR / "ask.html"
        return web.FileResponse(html_path)
