...
log = logging.getLogger("london.ingestors.planning")
...
# These are the numeric entity IDs for London local planning authorities.
# A subset covering key boroughs; the API also supports dataset-level queries.
LONDON_BOROUGHS = [
    # We query by dataset + date range rather than enumerating all 33 boroughs,
    # which is more resilient to ID changes.
]
