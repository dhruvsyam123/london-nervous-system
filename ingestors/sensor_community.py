...
log = logging.getLogger("london.ingestors.sensor_community")
    ...
    async def process(self, data: Any) -> None:
        ...
        for entry in data:
            ...
            for sv in entry.get("sensordatavalues", []):
                ...
                # Sanity: discard obviously broken readings (negative or > 1000 µg/m³)
                ...
                msg = AgentMessage(
                    ...
                    channel="#raw",
                    content=(
                        f"Citizen air sensor [{sensor_id}] {species}={value:.1f} µg/m³"
                    ),
                    data={
                        "sensor_id": sensor_id,
                        "sensor_type": sensor_type,
